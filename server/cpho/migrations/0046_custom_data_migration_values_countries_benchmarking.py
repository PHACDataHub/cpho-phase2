...
def benchmarking_countries_update(apps, schema_editor):
    ...
    if korea.exists():
        ...
        return
    england_wales = Country.objects.filter(name_en="England & Wales")
    if england_wales.exists():
        england_wales = england_wales.first()
    else:
        return
    uk_britain_ireland = Country.objects.filter(
        name_en="United Kingdom of Great Britain and Northern Ireland"
    )
    if uk_britain_ireland.exists():
        uk_britain_ireland = uk_britain_ireland.first()
    else:
        return
    slovakia = Country.objects.filter(name_en="Slovakia")
    if slovakia.exists():
        slovakia = slovakia.first()
    else:
        return
    ...
    # countries to delete: "Republic of Korea", "United Kingdom of Great Britain and Northern Ireland", "England & Wales", "Slovakia"
    if korea:
        korea.delete()
    if uk_britain_ireland:
        uk_britain_ireland.delete()
    if england_wales:
        england_wales.delete()
    if slovakia:
        slovakia.delete()
...
def update_benchmarking_values(apps, schema_editor):
    ...
    benchmarking_updates = {
        ...
        "total_deaths_per_1m": "deaths_per_million_inhabitants",
    }
...
class Migration(migrations.Migration):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # skip this migration when running tests
