...
class DataPointArgsInput(
    convert_serializer_to_input_type(DataPointSerializer)
):
    pass
...
class Query(graphene.ObjectType):
    ...
    indicators_by_id = graphene.List(
        IndicatorType, ids=graphene.List(graphene.Int)
    )
    ...
    def mutate(cls, root, info, **kwargs):
        ...
        try:
            ...
            return ModifyIndicator(
                indicator=indicatorObj, dataPoints=dataPoints, success=True
            )

        except Indicator.DoesNotExist:
            print("Could not find")
            return ModifyIndicator(
                indicator=None, dataPoints=None, success=False
            )
