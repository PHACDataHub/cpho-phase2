...
def add_metadata_to_all_logs_for_current_request(metadata_dict):
    ...
    # First, and more straightforward, binding structlog context vars adds to _all_ log output because
    # both our `logging` and `structlog` configs share the `structlog.contextvars.merge_contextvars`
    # processor

    # Second, and more obscurely, these bound context vars only apply to the _current_ request because
    # django_structlog.middlewares.RequestMiddleware clears all context vars at the end of its own
    # response logging handler. Not well documented, but expected to be stable. Clearing context vars
    # either on new request or when finished a response is recommended by structlog's own docs, and is
    # the responsibility of any framework-specific structlog wrapper
