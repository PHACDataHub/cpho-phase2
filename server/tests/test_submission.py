...
def test_submit_all_dimensions(vanilla_user, vanilla_user_client):
    ...
    resp = vanilla_user_client.post(url, {})
    assert resp.status_code == 302

    assert IndicatorDataSubmission.objects.count() == 1
    assert female_record.versions.last().is_hso_approved
    assert not female_record.versions.first().is_hso_approved
    assert male_record.versions.last().is_hso_approved
    assert canada_record.versions.last().is_hso_approved
